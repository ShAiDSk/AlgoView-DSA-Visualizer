...
// ✅ Clean Imports
import GraphCanvas, { type GraphRef } from "./GraphCanvas";
import {
  type Algorithm,
  type Stats,
  type SortingRun,
...
function App() {
  ...
  // --- NEW: Helper to update stats from Graph ---
  ...
  const handleStart = () => {
    ...
    setIsSorting(true);
    setLogs([]);
    setStats({ comparisons: 0, swaps: 0 }); // Reset stats

    if (selectedAlgo?.category === "Graph") {
      const runGraphAlgo = async () => {
        try {
          if (selectedAlgo.id === "bfs_graph") {
            await graphRef.current?.runBFS(speed, addLog, handleGraphStats);
          } else if (selectedAlgo.id === "dijkstra") {
            await graphRef.current?.runDijkstra(
              speed,
              addLog,
              handleGraphStats
            );
          } else if (selectedAlgo.id === "prim") {
            await graphRef.current?.runPrim(speed, addLog, handleGraphStats);
          }
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-slate-200 font-sans flex flex-col md:flex-row overflow-hidden selection:bg-blue-500/30">
      {/* Toasts */}
        ...
        <div className="grid grid-cols-2 gap-3">
          {selectedAlgo?.category === "Graph" ? (
            <button
              ...
              disabled={isSorting}
              className="flex items-center justify-center gap-2 py-3 bg-indigo-600/20 hover:bg-indigo-600/40 text-indigo-300 border border-indigo-500/30 rounded-xl font-semibold transition"
            >
            ...
            <button
              onClick={resetArray}
              disabled={isSorting}
              className="flex items-center justify-center gap-2 py-3 bg-slate-700/50 hover:bg-slate-700 rounded-xl font-semibold transition disabled:opacity-50 border border-white/5"
            >
          ...
          {!isSorting ? (
            <button
              onClick={handleStart}
              className="flex items-center justify-center gap-2 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 rounded-xl font-bold shadow-lg shadow-blue-900/40 transition active:scale-95"
            >
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 py-3 bg-red-500/10 text-red-400 border border-red-500/50 hover:bg-red-500/20 rounded-xl font-bold transition animate-pulse"
            >
      ...
      <div className="flex-1 flex flex-col relative bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-800 to-[#0f172a]">
        <div className="flex-1 flex items-end justify-center px-4 pb-10 pt-4 gap-[2px] relative w-full h-full">
            ...
            <div className="w-full h-full flex items-end justify-center gap-[2px] px-10">
              {array.map((val, idx) => {
                ...
                if (activeIndices.includes(idx))
                  colorClass =
                    "bg-gradient-to-t from-pink-500 to-rose-500 shadow-[0_0_15px_rgba(236,72,153,0.6)] z-10";
                else if (
                  selectedAlgo?.id === "binary" &&
                  (idx < activeIndices[1] || idx > activeIndices[2])
                )
                  colorClass = "bg-slate-800 opacity-30";
                return (
                  <motion.div
                    key={idx}
                    layout={!isSorting}
                    initial={{ height: 0, opacity: 0 }}
                    animate={{ height: `${val}px`, opacity: 1 }}
                    transition={{ type: "spring", damping: 20, stiffness: 300 }}
                    className={`flex-1 rounded-t-sm ${colorClass}`}
                    style={{ maxWidth: "30px", minHeight: "6px" }}
                  />
                );
              })}
            </div>
          ...
          <div className="absolute top-4 right-4 w-72 h-48 bg-black/80 backdrop-blur-md rounded-lg border border-white/10 flex flex-col overflow-hidden shadow-2xl z-20">
            <div className="bg-white/5 p-2 px-3 text-xs font-bold text-slate-400 flex items-center gap-2 border-b border-white/5">
            ...
            <table className="w-full text-left text-sm text-slate-400">
              <thead className="bg-black/20 sticky top-0 text-xs uppercase font-semibold text-slate-500">
                <tr>
                  ...
                  <th className="p-3 text-right pr-6">Time</th>
                </tr>
              </thead>
              <tbody>
                {history.map((run) => (
                  ...
                  >
                    <td className="p-3 pl-6 font-medium text-blue-300">
                      {run.algorithm}
                    </td>
                    <td className="p-3">{run.comparisons}</td>
                    <td className="p-3">{run.swaps}</td>
                    <td className="p-3 text-right pr-6 font-mono text-xs opacity-50">
